<?php
	...
	*/
	...
	//if(count($rowClient)>0){
			...
			$results = $mysqli->query('INSERT INTO data_antrian (waktu,counter,status,nomor,kodebooking,jenis_antrian_poliklinik,sesi) VALUES ("'.date("Y-m-d H:i:s").'",'.$loket.',3,'.$jmlCountId.',"'.$KodeBookings.'","3","'.$sesi_antrian.'")');
			...
			$rstCountIdTotal = $mysqli->query('SELECT count(*) as count FROM data_antrian WHERE id '.$filter_waktu);
			$rowCountIdTotal = $rstCountIdTotal->fetch_array();
			if($rowCountIdTotal['count']>0){
				$jmlCountIdTotal = $rowCountIdTotal['count'];
			}else{
				$jmlCountIdTotal = 0;
			}
			//----
