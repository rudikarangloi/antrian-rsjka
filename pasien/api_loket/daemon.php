<?php
	...
	//if(count($rowClient)>0){
			...
			/*
			Jika pasien baru dan terdaftar di hari yg sama saat pembuatan kodebooking, maka ambil kode booking dari tabel tb_biodata
			tb_biodata harus ditambah field kodebooking
				ALTER TABLE tb_biodata ADD COLUMN kodebooking VARCHAR(20) DEFAULT '' NULL AFTER keterangan_kartu; 

			*/
			           
			$KodeBookings = KodeBooking();

			//SELECT kodebooking FROM tb_biodata WHERE No_KTP='$fNik' AND DATE(Recorded) = CURDATE() 

			$rst = $mysqli->query("
									SELECT a.kodebooking FROM tb_biodata a INNER JOIN data_antrian b
									ON a.kodebooking=b.kodebooking WHERE STATUS = 2
									AND a.No_KTP='$fNik' AND DATE(a.Recorded) = CURDATE() 
									");
			$row = $rst->fetch_array();
			if($row['kodebooking']){
				$KodeBookings = $row['kodebooking'];				
			}
			
			$results = $mysqli->query('
				INSERT INTO data_antrian (waktu,counter,status,nomor,kodebooking,nomorkartubpjs,nik,jenis_antrian_poliklinik,sesi,kodedokter) 
				VALUES ("'.date("Y-m-d H:i:s").'",'.$loket.',3,'.$jmlCountId.',"'.$KodeBookings.'","'.$noBPJS.'","'.$noKTP.'",
				"'.$jenis_antrian_poliklinik.'","'.$sesi_antrian.'","'.$kodedokter.'")');
	...
	//$data['kodepoli'] = $loket;   //  Ambil dari tabel client_antrian field kdpoli
	$data['namapoli'] = $nama_loket;
	$data['kodebooking'] = $KodeBookings;
	
	//PR
	$estimasidilayani = strtotime(date('Y-m-d H:i:s', strtotime('+3 hours')));
	
	$rstClient = $mysqli->query("SELECT * FROM client_antrian WHERE client='$loket' ");
	$rowClient = $rstClient->fetch_array();
	if($rowClient['id']){
	...
	$rstCount = $mysqli->query("SELECT count(*) as count FROM data_antrian_detail 
								WHERE jenisantrianpoliklinik='1' ");
