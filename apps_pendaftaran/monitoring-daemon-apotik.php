<?php 
	...
	*/
		...
		*/
		...
		$sql = 'SELECT max(nomor) as nomor FROM data_antrian_apotik WHERE status=2 '.$sql_loket.$filter_waktu;
		...
		//## 4. Cari banyak data di tabel data_antrian yg status = 1 [Tidak pernah ada]
			...
			$sql = 'SELECT * FROM data_antrian_apotik WHERE status=0 '.$sql_loket.$filter_waktu.' ORDER BY waktu ASC LIMIT 1';
			//echo $sql."<P>";
