	...
	*/
		...
		*/
		...
		while ($cl = $client->fetch_array()) {
			...
			$rst = $mysqli->query('SELECT max(id) as id FROM data_antrian WHERE counter ='. $cl['client'] .' and status=2'.$filter_waktu); // execution
			$row = $rst->fetch_array();
			if ($row['id']==NULL) {
				$id=0;
			} else {
		...
		*/
		...
		if ($count){
			...
			$result = $mysqli->query('SELECT id, counter FROM data_antrian WHERE status=0 '.$filter_waktu.' ORDER BY waktu ASC LIMIT 1'); // execution
